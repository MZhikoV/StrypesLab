...
*different key words for name functions
**longer than 1 char function names
***empty fields between elements
****variables with same name in definition (it is discussable if in the expression is possible to have)
****different synthax - use different symbols for delimiters, for ':', etc.
*****missing synthax elements - parenthesis(opening or closing or both), ',', ':', etc.
******on run command, parameters to be with different values from just 0 and 1 (if the values are greater than 1 or <0, all will be 'true)
*******on run commant to give fewer or greater number of variables;
********on all command to give non-existing name;
*/
