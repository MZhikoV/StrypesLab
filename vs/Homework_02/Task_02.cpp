...
char* stringReplace(const char* haystack, const char* needle, const char* replace) {
    ...
    if (result) {
        ...
        for (int i = 0; i < occurancesCount; i++) {
            ...
            strcpy_s(currPtr, (strlen(haystack) - strlen(findString)), haystack);    //copy the substring before the occurance
            currPtr += strlen(haystack) - strlen(findString);   //move the ptr to the end of the new string           
            strcpy_s(currPtr, strlen(replace), replace);    //copy the replace string in the new string at current position
        ...
        strcpy_s(currPtr, strlen(haystack), haystack);   //copy the remainder of the main string in the new string
