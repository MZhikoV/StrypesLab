...
char* createProgressBar(float currProgress = 0.0, bool flagPercent = true, int totArrLength = 50,
    char endLeft = '<', char endRight = '>', char fillSymb = '+', char emptySymb = '-');

//creating Progress bar using std::string
std::string createProgressBarV02(float currProgress = 0.0, bool flagPercent = true, int totArrLength = 50,
    char endLeft = '<', char endRight = '>', char fillSymb = '+', char emptySymb = '-');
...
int main() {
    ...
    for (int i = 0; i <= barSections; i++) {
        ...
        std::cout << std::endl << "Using default function parameters:" << std::endl;
        std::cout << "Using dynamic memory allocation:" << std::endl;
        char* progressBarD = createProgressBar(currProgress);
        std::cout << progressBarD << std::endl;
        free(progressBarD);

        std::cout << "Using std::string:" << std::endl;
        std::string progressBarV02D = createProgressBarV02(currProgress);
        std::cout << progressBarV02D << std::endl;
...
//creating Progress bar using dynamic memory allocation
char* createProgressBar(float currProgress, bool flagPercent, int totArrLength,
    char endLeft, char endRight, char fillSymb, char emptySymb) {
    ...
    if (arrBar) {
        ...
        if (flagPercent) {
            ...
            int centerPoint = (totArrLength / 2 - 1);
            int percentDefl = 0;

            if (progressCheck < 10) {
                arrBar[centerPoint] = static_cast<char>(progressCheck + 48);
                percentDefl++;
            }
